...
class Options_Machine {
        ...
        foreach ($options as $value) {
            ...
            if (!empty($smof_data) || !empty($data)) {
                ...
                if ($value['type'] != "heading") {
                    ...
                    $data_class_show = isset($value['data-class_show']) ? ' data-class_show="' . esc_attr($value['data-class_show']) . '"' : '';
                    
                    //hide items in checkbox group
                    $fold = '';
                    if (array_key_exists("fold", $value)) {
                        $fold = (isset($smof_data[$value['fold']]) && $smof_data[$value['fold']]) ? "f_" . $value['fold'] . " " : "f_" . $value['fold'] . " temphide ";
                    }

                    $output .= '<div id="section-' . $value['id'] . '" class="' . $fold . 'section section-' . $value['type'] . ' ' . $class . '"' . $data_class_show . '>' . "\n";
                ...
                switch ($value['type']) {
                    ...
                    case 'typography':
                        ...
                        if (isset($typography_stored['face'])) {
                            ...
                            $faces = array(
                                'arial' => esc_html__('Arial', 'elessi-theme'),
                                'verdana' => esc_html__('Verdana, Geneva', 'elessi-theme'),
                                'trebuchet' => esc_html__('Trebuchet', 'elessi-theme'),
                                'georgia' => esc_html__('Georgia', 'elessi-theme'),
                                'times' => esc_html__('Times New Roman', 'elessi-theme'),
                                'tahoma' => esc_html__('Tahoma, Geneva', 'elessi-theme'),
                                'palatino' => esc_html__('Palatino', 'elessi-theme'),
                                'helvetica' => esc_html__('Helvetica', 'elessi-theme')
                            );
                        ...
                        if (isset($typography_stored['style'])) {
                            ...
                            $styles = array(
                                'normal' => esc_html__('Normal', 'elessi-theme'),
                                'italic' => esc_html__('Italic', 'elessi-theme'),
                                'bold' => esc_html__('Bold', 'elessi-theme'),
                                'bold italic' => esc_html__('Bold Italic', 'elessi-theme')
                            );
                    ...
                    case 'border':
                        ...
                        $styles = array(
                            ...
                            'solid' => esc_html__('Solid', 'elessi-theme'),
                            'dashed' => esc_html__('Dashed', 'elessi-theme'),
                            'dotted' => esc_html__('Dotted', 'elessi-theme')
                        );
                    ...
                    case 'backup':
                        ...
                        if (!isset($backup['backup_log'])) {
                            $log = esc_html__('No backups yet.', 'elessi-theme');
                        } else {
    ...
    public static function optionsframework_media_uploader_function($id, $std, $mod) {
        ...
        if (function_exists('wp_enqueue_media')) {
            ...
            $output .= '<p class="upload-notice"><i>' . esc_html__('Upgrade your version of WordPress for full media support.', 'elessi-theme') . '</i></p>';
        }
    ...
    public static function optionsframework_slider_function($id, $std, $oldorder, $order) {
        ...
        if (!empty($val['title'])) {
            ...
            $slider .= '<li><div class="slide_header"><strong>' . esc_html__('Slide', 'elessi-theme') . ' ' . $order . '</strong>';
        }
        ...
        if (!empty($val['url'])) {
            ...
            $slider .= '<img class="of-option-image" id="image_' . $id . '_' . $order . '" src="' . $val['url'] . '" />';
            $slider .= '</a>';
        }
        $slider .= '</div>';
