...
jQuery(document).ready(function($) {
     ...
     * Compare Images
     */
    nasa_init_compare_images($);
    $('body').on('nasa_init_compare_images', function() {
        nasa_init_compare_images($);
    });
...
function nasa_init_compare_images($) {
    if ($('.nasa-compare-images').length) {
        $('.nasa-compare-images').each(function() {
            var _this = $(this);

            if (!$(_this).hasClass('nasa-inited')) {
                $(_this).find('img').each(function() {
                    var _srcLazy = $(this).attr('data-lazy-src');
                    if (_srcLazy) {
                        $(this).attr('src', _srcLazy);
