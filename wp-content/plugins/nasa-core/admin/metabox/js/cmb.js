...
window.CMB = (function (window, document, $, undefined) {
    ...
    cmb.init = function () {
        ...
        if ($repeatGroup.length) {
            $repeatGroup.filter('.sortable').each(function () {
                // Add sorting arrows
            ...
            }).on('click', '.shift-rows', cmb.shiftRows).on('cmb_add_row', cmb.emptyValue);
        }
