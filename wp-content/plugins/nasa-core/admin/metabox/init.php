...
class cmb_Meta_Box {
     ...
     * Get started
     */
    public function __construct($_meta_box) {
        $meta_box = self::set_mb_defaults($_meta_box);

        $allow_frontend = apply_filters('cmb_allow_frontend', true, $meta_box);

        if (!NASA_CORE_IN_ADMIN && !$allow_frontend) {
            return;
        }
    ...
    public static function autoload_helpers($class_name) {
        ...
        if (file_exists(NASA_CORE_PLUGIN_PATH . 'admin/metabox/helpers/' . $class_name . '.php')) {
            include_once NASA_CORE_PLUGIN_PATH . 'admin/metabox/helpers/' . $class_name . '.php';
        }
    }
    ...
    public function register_scripts() {
        ...
        // wp_register_script('cmb-datepicker', NASA_CORE_PLUGIN_URL . 'admin/metabox/js/jquery.datePicker.min.js');
        // wp_register_script('cmb-timepicker', NASA_CORE_PLUGIN_URL . 'admin/metabox/js/jquery.timePicker.min.js');
        wp_register_script('cmb-scripts', NASA_CORE_PLUGIN_URL . 'admin/metabox/js/cmb' . $min . '.js', $scripts, self::CMB_VERSION);

        wp_register_script('nasa-script-metabox', NASA_CORE_PLUGIN_URL . 'admin/metabox/js/nasa-script-metabox.js', $scripts);
        ...
        wp_localize_script('cmb-scripts', 'cmb_l10', apply_filters('cmb_localized_data', array(
            ...
            'up_arrow' => '[ UP ]&nbsp;',
            'down_arrow' => '&nbsp;[ DOWN ]',
            'check_toggle' => esc_html__('Select / Deselect All', 'nasa-core'),
        )));

        wp_register_style('cmb-styles', NASA_CORE_PLUGIN_URL . 'admin/metabox/style' . $min . '.css', $styles);
    ...
    public function do_scripts($hook) {
        ...
        if ($hook == 'post.php' || $hook == 'post-new.php' || $hook == 'page-new.php' || $hook == 'page.php') {
            ...
            wp_enqueue_script('nasa-script-metabox');
    ...
    public function add_post_enctype() {
        ...
        '<script>' .
            'jQuery(document).ready(function($){' .
                '$("#' . $this->form_id . '").attr("enctype", "multipart/form-data");' .
                '$("#' . $this->form_id . '").attr("encoding", "multipart/form-data");' .
            '});' .
        '</script>';
    }
    ...
    public function user_metabox() {
        if (!$this->_meta_box || 'user' != self::set_mb_type($this->_meta_box) || !apply_filters('cmb_show_on', true, $this->_meta_box)) {
            return;
        }
    ...
    public static function render_group($args) {
        ...
        if ($desc || $label) {
            echo '<tr><th>';
            echo $label ? '<h2 class="cmb-group-name">' . $label . '</h2>' : '';
            echo $desc ? '<p class="cmb_metabox_description">' . $desc . '</p>' : '';
            echo '</th></tr>';
        }
    ...
    public static function render_group_row($field_group, $remove_disabled) {
        ...
        echo '<tr><td class="remove-row" colspan="2"><button ' . $remove_disabled . 'data-selector="' . $field_group->id() . '_repeat" class="button remove-group-row alignright">' . $field_group->options('remove_button') . '</button></td></tr></table></td></tr>';
     ...
     * @param  string $object_type Type of object being saved. (e.g., post, user, or comment)
     */
    public static function save_fields($_meta_box, $object_id, $object_type = '') {
        $meta_box = self::set_mb_defaults($_meta_box);
     ...
     * This may need to be filtered for local Nasadow installations.
    ...
    public static function get_meta_box_url() {
        ...
        if (strtoupper(substr(PHP_OS, 0, 3)) === 'NASA') {
            // Nasadows
            $content_dir = str_replace('/', DIRECTORY_SEPARATOR, WP_CONTENT_DIR);
            $content_url = str_replace($content_dir, WP_CONTENT_URL, NASA_CORE_PLUGIN_PATH . 'includes/metabox');
            $cmb_url = str_replace(DIRECTORY_SEPARATOR, '/', $content_url);
        } else {
            $cmb_url = str_replace(
                array(WP_CONTENT_DIR, WP_PLUGIN_DIR), array(WP_CONTENT_URL, WP_PLUGIN_URL), NASA_CORE_PLUGIN_PATH . 'admin/metabox'
            );
        }
...
function cmb_metabox_form($meta_box, $object_id, $echo = true) {
    ...
    $form = sprintf($form_format, $meta_box['id'], $object_id, $form, esc_html__('Save', 'nasa-core'));
